#!/usr/bin/env bash
#
# Installs the TenshiSTEP SDDM login theme system-wide (requires sudo,
...
VARIANT="${1:-qt5}"
case "$VARIANT" in
  qt5) SRC="$HERE/TenshiSTEP" ;;
  qt6) SRC="$HERE/qt6/TenshiSTEP" ;;
  *) echo "unknown variant '$VARIANT' (use qt5 or qt6)"; exit 1 ;;
esac
DEST=/usr/share/sddm/themes/TenshiSTEP
