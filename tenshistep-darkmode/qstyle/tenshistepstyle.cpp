...
static const QColor kHi    (0x56, 0x5d, 0x67);
static const QColor kSh    (0x14, 0x16, 0x1a);
static const QColor kBlue  (0x6a, 0x5f, 0xd6);
static const QColor kButton(0x3b, 0x40, 0x48);
static const QColor kWindow(0x2b, 0x2e, 0x34);
static const QColor kGroove(0x20, 0x23, 0x29);
static const QColor kGreen (0x4a, 0x7a, 0x3a);
static const QColor kRadio (0x8a, 0x80, 0xf0);   // selected radio dot (lightened for dark base)
static const QColor kGlyph (0xdc, 0xdf, 0xe4);   // light foreground glyph (arrows) on dark wells
...
static void drawArrow(QPainter *p, const QRect &r, Qt::ArrowType dir,
                      const QColor &col = kGlyph, qreal grow = 1.0)
{
...
static QLinearGradient metalFill(const QRect &r)
{
    ...
    g.setColorAt(0.00, QColor(0xb8, 0xae, 0xf4));
    g.setColorAt(0.12, QColor(0xdd, 0xd6, 0xfa));   // specular sheen
    g.setColorAt(0.50, QColor(0x6a, 0x5f, 0xd6));
    g.setColorAt(0.54, QColor(0x5a, 0x4f, 0xc8));
    g.setColorAt(0.90, QColor(0x3d, 0x35, 0x9e));
    g.setColorAt(1.00, QColor(0x2a, 0x24, 0x78));
    return g;
}
...
{
    QLinearGradient g(r.topLeft(), r.bottomLeft());
    g.setColorAt(0.00, QColor(0x3a, 0x3f, 0x47));
    g.setColorAt(0.18, QColor(0x4a, 0x50, 0x5a));
    g.setColorAt(0.55, QColor(0x33, 0x38, 0x40));
    g.setColorAt(1.00, QColor(0x2b, 0x2f, 0x36));
    return g;
}
...
static void applyPalette(QPalette &pal)
{
    ...
    pal.setColor(QPalette::AlternateBase,   QColor(0x26, 0x28, 0x2d));
    pal.setColor(QPalette::Button,          kButton);
    pal.setColor(QPalette::Light,           kHi);
    pal.setColor(QPalette::Midlight,        QColor(0x45, 0x4b, 0x54));
    ...
    pal.setColor(QPalette::ToolTipBase,     QColor(0x33, 0x37, 0x3e));
    pal.setColor(QPalette::ToolTipText,     QColor(0xdc, 0xdf, 0xe4));
    pal.setColor(QPalette::Highlight,       kBlue);
    pal.setColor(QPalette::HighlightedText, QColor(Qt::white));
    pal.setColor(QPalette::Link,            QColor(0x9a, 0x90, 0xf0));
    pal.setColor(QPalette::LinkVisited,     QColor(0xb0, 0xa5, 0xd0));
    pal.setColor(QPalette::Disabled, QPalette::WindowText, QColor(0x9a, 0xa0, 0xa8));
    pal.setColor(QPalette::Disabled, QPalette::Text,       QColor(0x9a, 0xa0, 0xa8));
    pal.setColor(QPalette::Disabled, QPalette::ButtonText, QColor(0x9a, 0xa0, 0xa8));
}

// --- style ------------------------------------------------------------------

TenshiSTEPDarkmodeStyle::TenshiSTEPDarkmodeStyle() : QProxyStyle(QStringLiteral("Fusion")) {}

void TenshiSTEPDarkmodeStyle::polish(QPalette &palette) { applyPalette(palette); }

QPalette TenshiSTEPDarkmodeStyle::standardPalette() const
{
...
int TenshiSTEPDarkmodeStyle::pixelMetric(PixelMetric m, const QStyleOption *o, const QWidget *w) const
{
    switch (m) {
...
void TenshiSTEPDarkmodeStyle::drawPrimitive(PrimitiveElement pe, const QStyleOption *opt,
                                       QPainter *p, const QWidget *w) const
{
    ...
    case PE_IndicatorCheckBox: {
        // NeXT-style: grey RAISED bevel with a silvery-grey check
        ...
        if (opt->state & State_On) {
            ...
            p->save();
            p->setRenderHint(QPainter::Antialiasing, true);
            QPen pen(QColor(0xa6, 0xac, 0xb4), 2.2); pen.setCapStyle(Qt::RoundCap); pen.setJoinStyle(Qt::RoundJoin);
    ...
    case PE_IndicatorRadioButton: {
        ...
        p->save();
        p->setRenderHint(QPainter::Antialiasing, true);
        p->setBrush(QColor(0x20, 0x22, 0x26));
...
void TenshiSTEPDarkmodeStyle::drawControl(ControlElement ce, const QStyleOption *opt,
                                     QPainter *p, const QWidget *w) const
{
    switch (ce) {
...
QRect TenshiSTEPDarkmodeStyle::subControlRect(ComplexControl cc, const QStyleOptionComplex *opt,
...
void TenshiSTEPDarkmodeStyle::drawComplexControl(ComplexControl cc, const QStyleOptionComplex *opt,
                                            QPainter *p, const QWidget *w) const
{
    if (cc == CC_ScrollBar) {
    ...
    if (cc == CC_SpinBox) {
        // Fusion-like: recessed field + a flat button per arrow (no white bevel),
        // with slightly larger arrows; the fill darkens when the button is pressed.
        ...
        if (uw.width() > 3 && uw.height() > 3) { paintBevel(p, uw, upSunk   ? kGroove : kButton, false, kSh, false); drawArrow(p, uw.translated(0, upSunk   ? 1 : 0), Qt::UpArrow,   kGlyph, 1.5); }
        if (dw.width() > 3 && dw.height() > 3) { paintBevel(p, dw, downSunk ? kGroove : kButton, false, kSh, false); drawArrow(p, dw.translated(0, downSunk ? 1 : 0), Qt::DownArrow, kGlyph, 1.5); }
        return;
    }
