# TenshiSTEP-darkmode for KDE Plasma
...
> **Dark-mode variant.** This is the dark counterpart of the light `TenshiSTEP` theme: gunmetal chrome, near-black backgrounds, light text, and brighter metallic-indigo accents. It ships its **own full dark icon set** (`Icons=TenshiSTEP-darkmode`) — the light icons with the chiselled outline recoloured (`#1a1a1a` → `#cdd1d7`) so they read on dark backgrounds — at full parity with the light set. Everything else (colour scheme, Plasma style, Kvantum, QSS, QStyle, Aurorae, GTK, SDDM, Global Theme, Plymouth, cursors, wallpaper) has its own dark build.
...
|-------|------|----------------|
| Color scheme | `color-schemes/TenshiSTEP-darkmode.colors` | App colors — NeXT grays, white views, dark selection |
| Window decoration | `aurorae/TenshiSTEP-darkmode/` | Aurorae title bars & borders with beveled square buttons |
| Konsole scheme | `konsole/TenshiSTEP-darkmode.colorscheme` | Terminal palette (black on white, muted ANSI) |
| Icon theme | `icons/TenshiSTEP-darkmode/` | OPENSTEP-inspired **colour** icons with the chiselled bevel; inherits Breeze for the rest |
| SDDM login theme | `sddm/TenshiSTEP-darkmode/` (Qt5) · `sddm/qt6/TenshiSTEP-darkmode/` (Qt6) | OPENSTEP-style QML greeter — chiselled panel, indigo title bar, colour power buttons |
| Plasma Style | `plasma/desktoptheme/TenshiSTEP-darkmode/` | NeXT chiselled FrameSvg widgets (panels, plasmoids, buttons, fields, tooltips) |
| Global Theme | `plasma/look-and-feel/org.tenshistep.darkmode.desktop/` | Look-and-Feel that applies the whole set + boot splash + logout screen |
| Kvantum theme | `kvantum/TenshiSTEP-darkmode/` | NeXT app-widget style (SVG) for the Kvantum QStyle engine |
| Qt style sheet | `qt-style/TenshiSTEP-darkmode.qss` | Lightweight NeXT-beveled widget QSS for any Qt app |
...
SVGs under the matching `icons/TenshiSTEP-darkmode/<context>/scalable/` folder to grow
coverage.
...
cp color-schemes/TenshiSTEP-darkmode.colors      ~/.local/share/color-schemes/
cp -r aurorae/TenshiSTEP-darkmode                 ~/.local/share/aurorae/themes/
cp konsole/TenshiSTEP-darkmode.colorscheme        ~/.local/share/konsole/
cp -R icons/TenshiSTEP-darkmode                   ~/.local/share/icons/
```

## Apply

- **Colors:** System Settings → Colors → *TenshiSTEP-darkmode*
  (or `plasma-apply-colorscheme TenshiSTEP-darkmode`)
- **Window decoration:** System Settings → Window Decorations → *TenshiSTEP-darkmode*
- **Konsole:** Konsole → Settings → Edit Current Profile → Appearance → *TenshiSTEP-darkmode*
- **Icons:** System Settings → Icons → *TenshiSTEP-darkmode*
...
The login greeter lives under `sddm/TenshiSTEP-darkmode/` (a self-contained Qt Quick
...
OPENSTEP-blue gradient. See `sddm/TenshiSTEP-darkmode/preview.png`.
...
```ini
[Theme]
Current=TenshiSTEP-darkmode
```

Preview it without logging out:

```bash
sddm-greeter --test-mode --theme /usr/share/sddm/themes/TenshiSTEP-darkmode
```

Background and panel/title colours are configurable in
`sddm/TenshiSTEP-darkmode/theme.conf` (set `background=` to a wallpaper path, or
...
Qt5 one (`sddm/TenshiSTEP-darkmode/`) and a Qt6 one (`sddm/qt6/TenshiSTEP-darkmode/`, with
...
`plasma/look-and-feel/org.tenshistep.darkmode.desktop/` is a **Global Theme**. Its
`contents/defaults` wires the whole set together — TenshiSTEP-darkmode colour scheme,
icon theme, Plasma Style, and Aurorae decoration — so one click applies
everything:

```bash
lookandfeeltool -a org.tenshistep.darkmode.desktop     # apply
...
lock screen automatically picks up the TenshiSTEP-darkmode **colour scheme**, so it's
...
export QT_STYLE_OVERRIDE=TenshiSTEP-darkmode       # or System Settings -> Application Style
```
...
```bash
kvantum/install-kvantum.sh          # copies to ~/.config/Kvantum/TenshiSTEP-darkmode
kvantummanager --set TenshiSTEP-darkmode
# then set the application style to "Kvantum" (System Settings / qt5ct / qt6ct)
```

`kvantum/TenshiSTEP-darkmode/TenshiSTEP-darkmode.svg` (488 elements) is generated by
`tools/gen_kvantum.py`; `TenshiSTEP-darkmode.kvconfig` maps each widget to it.

**Qt Style Sheet** (`qt-style/TenshiSTEP-darkmode.qss`) — a lighter, engine-free
alternative that works in any Qt app: point qt5ct/qt6ct → *Appearance →
Style Sheets* at it, or launch an app with `-stylesheet TenshiSTEP-darkmode.qss`.
...
- `TenshiSTEP-darkmoderc` — geometry: border thickness, title height, button size.
...
- **Title-bar height / button alignment** — tune `TitleHeight`,
  `ButtonHeight`, and `ButtonMarginTop` in `TenshiSTEP-darkmoderc`.
...
- **Border thickness** — `BorderLeft/Right/Bottom` in `TenshiSTEP-darkmoderc`.
