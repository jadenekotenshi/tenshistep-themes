#!/usr/bin/env python3
"""
Colorize the TenshiSTEP-zirconium icon theme with an OPENSTEP 4.2-inspired palette.

NOTE: the zirconium variant reuses the light theme's icon set UNCHANGED (its
content areas stay the same near-white as light, so the existing chiselled
outlines are already correct) -- this script is carried along for parity with
the other variants' tools/, but is not run as part of building zirconium.
...
ICONS = os.path.join(os.path.dirname(__file__), '..', 'icons', 'TenshiSTEP-zirconium')
